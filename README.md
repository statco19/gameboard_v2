# gameboard_v2
## 프로젝트 주제
League of Legends(LOL) API를 이용한 게임보드 제작

## 프로젝트 개요
LOL API로부터 LOL 솔로랭크 대전기록 데이터를 가져와 해당 패치의 승률, 밴률, 픽률을 구하고 각종 지표를 시각화하여 한눈에 메타를 살펴볼 수 있는 대시보드 제작하기

## 프로젝트 기술 스택
- AWS (AWC ECS, AWS EMR, AWS S3)
- Python (Python, Pandas, Pyspark)
- Spark (Spark SQL, Spark ML)
- ElasticSearch (ElasticSearch, Kibana)
- Javascript

# Architecture
<img width="950" alt="image" src="https://user-images.githubusercontent.com/70125217/166094148-e3640926-354c-4d14-8126-a69137e4d6a3.png">
### LOL-API & KAFKA>
lol-api.py
producer.py
consumer.py
### SPARK & ELASTICSEARCH
gameboard-indicators-analysis.ipynb
gameboard-indicators.py
gameboard-rate.py


# Demo
<img width="1437" alt="KakaoTalk_20220429_181002700" src="https://user-images.githubusercontent.com/70125217/166094171-e21a0524-ee67-4332-a2c1-203a4fec8184.png">

<img width="1324" alt="Screen Shot 2022-03-21 at 8 50 23 PM" src="https://user-images.githubusercontent.com/71916314/162386090-aa9e379c-fb1a-4f23-9636-8097953c1076.png">
